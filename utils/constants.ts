export const Skill_data: Skill_data[] = [
  {
    skill_name: "Html 5",
    Image: "/html.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Css",
    Image: "/css.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Java Script",
    Image: "/js.png",
    width: 65,
    hight: 65,
  },
  {
    skill_name: "Tailwind Css",
    Image: "/tailwind.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "React",
    Image: "/react.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Redux",
    Image: "/redux.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "React Query",
    Image: "/reactquery.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Type Script",
    Image: "/ts.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Next js 13",
    Image: "/next.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Next Auth",
    Image: "https://next-auth.js.org/img/logo/logo-sm.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Json Web Tocken",
    Image:
      "https://seeklogo.com/images/J/json-web-tokens-jwt-io-logo-C003DEC47A-seeklogo.com.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Framer Motion",
    Image: "/framer.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Stripe Payment",
    Image: "/stripe.webp",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Node js",
    Image: "/node-js.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Express js",
    Image: "/express.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Mongo db",
    Image: "/mongodb.png",
    width: 40,
    hight: 40,
  },
  {
    skill_name: "Fire base",
    Image: "/Firebase.png",
    width: 55,
    hight: 55,
  },
  {
    skill_name: "Postger SQL",
    Image: "/postger.png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "My SQL",
    Image: "/mysql.png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Prisma",
    Image: "/prisma.webp",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Graphql",
    Image: "/graphql.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "React Native",
    Image: "/ReactNative .png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Tauri",
    Image: "/tauri.svg",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Docker",
    Image: "/docker.webp",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Nigx",
    Image:
      "https://i0.wp.com/thinkground.studio/wp-content/uploads/2019/04/190415_Nginx-Logo.png?fit=423%2C401&ssl=1",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Figma",
    Image: "/figma.png",
    width: 50,
    hight: 50,
  },
  {
    skill_name: "Web Scoket",
    Image: "https://avatars.githubusercontent.com/u/10566080?s=200&v=4",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Cloundary",
    Image: "https://cdn.worldvectorlogo.com/logos/cloudinary-2.svg",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Go",
    Image: "/go.png",
    width: 60,
    hight: 60,
  },
  {
    skill_name: "Gin gonic",
    Image: "https://raw.githubusercontent.com/gin-gonic/logo/master/color.png",
    width: 50,
    hight: 50,
  },
  {
    skill_name: "Git",
    Image: "https://git-scm.com/images/logos/downloads/Git-Icon-1788C.png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Npm",
    Image: "https://authy.com/wp-content/uploads/npm-logo.png",
    width: 70,
    hight: 70,
  },
];

export const Frontend_skill: Skill_data[] = [
  {
    skill_name: "Html 5",
    Image: "/html.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Css",
    Image: "/css.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Java Script",
    Image: "/js.png",
    width: 65,
    hight: 65,
  },
  {
    skill_name: "Tailwind Css",
    Image: "/tailwind.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Material UI",
    Image: "/mui.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "React",
    Image: "/react.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Redux",
    Image: "/redux.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "React Query",
    Image: "/reactquery.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Type Script",
    Image: "/ts.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Next js 13",
    Image: "/next.png",
    width: 80,
    hight: 80,
  },
];

export const Backend_skill: Skill_data[] = [
  {
    skill_name: "Node js",
    Image: "/node-js.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Express js",
    Image: "/express.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Mongo db",
    Image: "/mongodb.png",
    width: 40,
    hight: 40,
  },
  {
    skill_name: "Fire base",
    Image: "/Firebase.png",
    width: 55,
    hight: 55,
  },
  {
    skill_name: "Postger SQL",
    Image: "/postger.png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "My SQL",
    Image: "/mysql.png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Prisma",
    Image: "/prisma.webp",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Graphql",
    Image: "/graphql.png",
    width: 80,
    hight: 80,
  },
];

export const Full_stack: Skill_data[] = [
  {
    skill_name: "React Native",
    Image: "/ReactNative .png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Tauri",
    Image: "/tauri.svg",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Docker",
    Image: "/docker.webp",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Nigx",
    Image:
      "https://i0.wp.com/thinkground.studio/wp-content/uploads/2019/04/190415_Nginx-Logo.png?fit=423%2C401&ssl=1",
    width: 60,
    hight: 60,
  },
  {
    skill_name: "Figma",
    Image: "/figma.png",
    width: 50,
    hight: 50,
  },
  {
    skill_name: "Web Scoket",
    Image: "https://avatars.githubusercontent.com/u/10566080?s=200&v=4",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Gin gonic",
    Image: "https://raw.githubusercontent.com/gin-gonic/logo/master/color.png",
    width: 50,
    hight: 50,
  },
];

export const Other_skill: Skill_data[] = [
  {
    skill_name: "Next Auth",
    Image: "https://next-auth.js.org/img/logo/logo-sm.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Json Web Tocken",
    Image:
      "https://seeklogo.com/images/J/json-web-tokens-jwt-io-logo-C003DEC47A-seeklogo.com.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Framer Motion",
    Image: "/framer.png",
    width: 80,
    hight: 80,
  },
  {
    skill_name: "Stripe Payment",
    Image: "/stripe.webp",
    width: 80,
    hight: 80,
  },
];

export const Other_skill2: Skill_data[] = [
  {
    skill_name: "Cloundary",
    Image: "https://cdn.worldvectorlogo.com/logos/cloudinary-2.svg",
    width: 70,
    hight: 70,
  },

  {
    skill_name: "Git",
    Image: "https://git-scm.com/images/logos/downloads/Git-Icon-1788C.png",
    width: 70,
    hight: 70,
  },
  {
    skill_name: "Npm",
    Image: "https://authy.com/wp-content/uploads/npm-logo.png",
    width: 70,
    hight: 70,
  },
];
export const Other_skill3: Skill_data[] = [
  {
    skill_name: "Go",
    Image: "/go.png",
    width: 60,
    hight: 60,
  },
];

export const Project: Project_data[] = [
  {
    title: "Manoj",
    video_key: "LUT276dNpRs",
    say_xi: "Introduction",
    say_xi_blog:
      "Manoj is on a wheelchair and wants to become an international rifle-shooting star. Help him get a world class rifle and realise his dreams.",
    technology_feature: [
      "Rifle Shooter",
      "Athlete",
      "Para Olympics",
      "Under - 18",
      "Stripe",
      "Framer Motion",
      "Type Script",
      "Redux",
    ],
    about_this_app:
      "Manoj comes from a poor household.  His dad is an autorickshaw driver and mom is a housewife, but a home full of love whose family members work together to make his life better. Despite the limitations, Manoj and his family make sure they attend every awareness program on injury prevention without fail",
    design_blog: " Ui/Ux Desing & All Code Written by Me  ",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I am use NPM",
    package: [
      "-> A professional ‘Peep Sight’ rifle that will help him get better shots with improved accuracy.",
      "-> The Walther Model LG 400 Expert Anatomic (Laminated Wood Stock) is an industry standard match rifle. It lightweight and great for Manoj to use.",
      "-> Plus a professional shooting jacket",
      "-> A pair of shooting gloves",
      "-> 20 tins of pellets for practice",
      // "maath",
      // "react-redux",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/LUT276dNpRs",
    githhub_link: "",
    frontend_dowload_link:
      "",
    backend_dowload_link: "",
    project_image: "/manoj.jpg",
    project_blog:
      "Manoj is on a wheelchair and wants to become an international rifle-shooting star. Help him get a world class rifle and realise his dreams.",
    Type: "Next js 13",
    indevelopment: false,
  },
  {
    title: "Gaurav Nandrajog",
    video_key: "gwHrVHnW1DA",
    say_xi: "Introduction ",
    say_xi_blog:
      "Gaurav Nandrajog, a 32-year-old squash champion from New Delhi is all set to get back in the game and needs funds for it!",
    technology_feature: [
      // "Tailwind",
      // "React",
      // "Redux",
      // "Next js 13",
      // "Type Script",
      // "Node js",
      "Squash",
      "Male",
      // "Firebase",
      "Indian",
    ],

    about_this_app:
      "I started learning to play squash when I was 13. After training for a year, I won the UNDER-15 nationals and went on to become the NO 1 PLAYER IN INDIA in the Under-17 category. I was ranked no 2 in the UNDER -19 and was seen as a boy wonder in the world of squash.",
    design_blog:
      "",
    dependencies_blog:
      "",
    package: [
      "I want to raise INR 12 L through this campaign to take part in international events. My total funding needs for the year is closer to INR 15L approximately so you can continue funding this campaign even after my goal has been achieved. Below is an estimation of the total expenses that I will incur through the year for just International events. ",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/gwHrVHnW1DA",
    githhub_link: "",
    frontend_dowload_link:
      "",
    backend_dowload_link:
      "",
    project_image: "/gaurav_nandrajog.png",
    project_blog:
      "Gaurav Nandrajog, a 32-year-old squash champion from New Delhi is all set to get back in the game and needs funds for it!",
    Type: "Next js 13",
    indevelopment: false,
  },
  {
    title: "Mythri Maddisetty",
    video_key: "gwHrVHnW1DA",
    say_xi: "Introduction ",
    say_xi_blog:
      "I am 16-years-old and a Grade 11 student at Chirec Intl School (IB), Hyderabad. This campaign of mine aims to empower the dreams of Shivamshika to become a chess champion.",
    technology_feature: [
      // "Tailwind",
      // "React",
      // "Redux",
      // "Next js 13",
      "Under - 18",
      // "Node js",
      // "Express js",
      "chess",
      "Female",
      "Stripe",
    ],

    about_this_app:
      "A Real Time Full Stack FoodOrder & Admin Dashboard application User Can Order and Contain Payment For Stripe And AdminDashBoard Feature Admin can make Product CRUD and Responsive Desing",
    design_blog:
      "desing and all of ui create by my own ideal , conatin Backend ",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I am use Yarn",
    package: [
      "We need a total of INR 50,000to make Shivamshika’s dream a reality. We aim to raise  50,000 OR MORE through crowdfunding. ",
      "Training - 12000",
      "Travel - 12000",
      "Tournament Fees - 3000",
      "Accomodation - 18000",
      "Miscellaneous - 5000",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/gwHrVHnW1DA",
    githhub_link: "",
    frontend_dowload_link:
      "/AdminDashboard/archive/refs/heads/main.zip",
    backend_dowload_link:
      "/DeshboardBackend/archive/refs/heads/main.zip",
    project_image: "/mythri.jpg",
    project_blog:
      "I am 16-years-old and a Grade 11 student at Chirec Intl School (IB), Hyderabad. This campaign of mine aims to empower the dreams of Shivamshika to become a chess champion.",
    Type: "Next js 13",
    indevelopment: false,
  },
  {
    title: "Bhakti Sharma",
    video_key: "yF1oJ7wzvWY",
    say_xi: "Introduction ",
    say_xi_blog:
      "Decorated open-water swimmer, Bhakti Sharma, hopes to put India in the spotlight at the 2020 Tokyo Olympic Games!",
    technology_feature: [
      "Next js 13",
      "Graphql",
      "Stripe",
    ],

    about_this_app:
      "A Real Time Full Stack FoodOrder & Admin Dashboard application User Can Order and Contain Payment For Stripe And AdminDashBoard Feature Admin can make Product CRUD and Responsive Desing",
    design_blog:
      "desing and all of ui create by my own ideal , conatin Backend ",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I am use Yarn",
    package: [
      "@apollo/client",
      "@emotion/react",
      "@emotion/styled",
      "@mui/material",
      "@mui/x-data-grid",
      "@nivo/bar",
      "@nivo/core",
      "@nivo/line",
      "@nivo/pie",
      "@reduxjs/toolkit",
      "@stripe/stripe-js",
      "@uiball/loaders",
      "encoding",
      "firebase",
      "graphql",
      "jwt-decode",
      "react-datepicker",
      "next-themes",
      "next-auth",
      "swiper",
      "uuid",
      "redux",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/yF1oJ7wzvWY",
    githhub_link: "",
    frontend_dowload_link:
      "/AdminDashboard/archive/refs/heads/main.zip",
    backend_dowload_link:
      "/DeshboardBackend/archive/refs/heads/main.zip",
    project_image: "/bhakti.jpg",
    project_blog:
      "Decorated open-water swimmer, Bhakti Sharma, hopes to put India in the spotlight at the 2020 Tokyo Olympic Games!",
    Type: "Next js 13",
    indevelopment: false,
  },

  {
    title: "Adventure Beyond Barriers ",
    video_key: "R90bxY4UwPg",
    say_xi: "Xello My Frends 💚",
    say_xi_blog:
      "This is my First AI Project data Coming From Opend AI , You can asked like RealTime Chat GPT   ",
    technology_feature: [
      // "html",
      // "css",
      "Tailwind",
      // "React",
      // "Redux",
      // "Next js 13",
      // "Type Script",
      // "Firebase",
      "Stripe",
    ],

    about_this_app:
      "This Appp Contain Chate Create , Read , Update , Delete and Google Authentication , Microsoft Authentication , JWT Authentication functionality ",
    design_blog: "Desing From Realworld ChatGPT",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I am use Npm ",
    package: [
      "@reduxjs/toolkit",
      "@uiball/loaders",
      "firebase",
      "firebase-admin",
      "nextjs-progressbar",
      "openai",
      "react-firebase-hooks",
      "react-icons",
      "react-redux",
      "react-simple-typewriter",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/R90bxY4UwPg",
    githhub_link: "",
    frontend_dowload_link:
      "/Chat_GPT4/archive/refs/heads/main.zip",
    backend_dowload_link: "",
    project_image: "/adventure.jpg",
    project_blog:
      "An indian Startup building trekking & mountain sports tourism in India",
    Type: "Next js 13",
    indevelopment: false,
  },
  {
    title: "Taran Williams",
    video_key: "v3LJMcWbcDQ",
    say_xi: "Hello my friends 👟",
    say_xi_blog:
      "Hi, I am Taran Williams, a 16-year-old boy from Bangalore. I was a student at Bethany High School, Koramangala. After a tough selection process, I have been selected to join the BARCA RESIDENCY ACADEMY, USA to pursue my goal of being a professional footballer and also to continue with my regular schooling.  This is the only official full-time youth residency academy in the world besides La Masia in Barcelona, Spain run by FC Barcelona. ",
    technology_feature: [
      // "Tailwind",
      "React",
      // "Redux",
      // "Next js 13",
      "Type Script",
      // "Node js",
      // "Express js",
      "Postger sql",
      // "Firebase",
      "Stripe",
    ],

    about_this_app:
      "A Real Time Full Stack Responsive Shoe Ecommerce & Admin Dashboard application User Can Order and Contain Real World Payment For Stripe And AdminDashBoard Feature Admin can make Product CRUD and Promote Product CRUD Admin Can look How Much User & Chart Pie , Chart line and Auth Google , Nike Auth",
    design_blog:
      "I Found Very Basic Ui Desing From Youtube , Then i get ideal To Make This All Desing By me 💚💚 ",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I am use NPM",
    package: [
      "@mui/material",
      "@nivo/core",
      "@nivo/line",
      "@nivo/pie",
      "@reduxjs/toolkit",
      "@splidejs/react-splide",
      "@stripe/stripe-js",
      "@uiball/loaders",
      "next-auth",
      "next-themes",
      "firebase",
      "javascript-time-ago",
      "jwt-decode",
      "react-circular-progressbar",
      "react-datepicker",
      "react-easy-crop",
      "react-icons",
      "react-hot-toast",
      "react-redux",
      "react-tilt",
      "uuid",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/v3LJMcWbcDQ",
    githhub_link: "",
    frontend_dowload_link:
      "/Nike_Shoes/archive/refs/heads/main.zip",
    backend_dowload_link:
      "/Nike_Backend/archive/refs/heads/main.zip",
    project_image: "/tarun.jpg",
    project_blog:
      "Hi, I am Taran Williams, a 16-year-old boy from Bangalore. I was a student at Bethany High School, Koramangala. After a tough selection process, I have been selected to join the BARCA RESIDENCY ACADEMY, USA to pursue my goal of being a professional footballer and also to continue with my regular schooling.  This is the only official full-time youth residency academy in the world besides La Masia in Barcelona, Spain run by FC Barcelona. ",
    Type: "Next js 13",
    indevelopment: false,
  },
  {
    title: "Tom Pouce",
    video_key: "6wzf2ho-kzI",
    say_xi: "Hello my friend! 👋🏼",
    say_xi_blog:
      "This is a Normal 3 js Responsive Profiol Website Constain Starts Mode , Background Music , Day and Light Mode , Contain youtube emb video like youtube Mobile Responsive desing Base on Vite ",
    technology_feature: [
      // "html",
      "css",
      "Javascript",
      "Type Script",
      // "Node js",
      // "Express js",
      "Postger sql",
      // "Tailwind",
      // "React",
      // "Redux",
      // "Threes js",
      // "Framer Motion",
    ],
    about_this_app:
      "This Responsive  Website is Make for showcase Project Profile ",
    design_blog:
      "very basic desing from My Friends , and I ReModified It All Code & Desing by Me ",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I use npm beacuse We use older npm package .",
    package: [
      " maath",
      " @react-three/fiber",
      " @emailjs/browser",
      " framer-motion",
      " @react-three/fiber ",
      " @reduxjs/toolkit ",
      " react-router-dom",
      " react-tilt",
      " react-simple-typewriter",
      " react-icons",
      " react-spinkit",
      " react-vertical-timeline-component",
      " three",
    ],
    conclusion_blog:
      "000",
    youtube_link: "https://youtu.be/6wzf2ho-kzI",
    githhub_link: "",
    frontend_dowload_link:
      "/Profiol_2/archive/refs/heads/main.zip",
    backend_dowload_link: "",
    project_image: "/tom_pouce.jpeg",
    project_blog:
      "A national level Golf player looking for funds to buy international grade equipments",
    Type: "React",
    indevelopment: false,
  },
  {
    title: "Osaka Karate Team",
    video_key: "IqYbA6ZZ-e4",
    say_xi: "Hi have a good day! ",
    say_xi_blog: "That is My  favourite game Website so i can make Clone one",
    technology_feature: [
      // "html",
      // "css",
      // "Javascript",
      // "Tailwind",
      // "React",
      // "Redux",
      // "Next js 13",
      "React Native",
      "Tauri",
    ],

    about_this_app:
      "Simple ApexLegnds Game Web application DropDown Feature Complex Responsive  and Contain Embed video",
    design_blog: "Desing From Apexlegends.com , I Recreate That Design",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I am use Npm",
    package: [
      "@reduxjs/toolkit",
      "next",
      "react-icons",
      "react-redux",
      "swiper",
      "typescript",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/IqYbA6ZZ-e4",
    githhub_link: "",
    frontend_dowload_link:
      "/ApexLegends.com/archive/refs/heads/main.zip",
    backend_dowload_link: "",
    project_image: "/karate.jpg",
    project_blog:
      "We Osaka Karate team wish to raise funds for providing world class facilities for our championship team",
    Type: "Next js 13",
    indevelopment: false,
  },
  {
    title: "Keshav Kumar ",
    video_key: "cLKsNVSyPMM",
    say_xi: "Hello my Friends 🎉",
    say_xi_blog:
      "Fullstack Social Media Project With  React , Tailwind css and My First Node js express Server ☘️ ",
    technology_feature: [
      // "html",
      // "css",
      // "Javascript",
      // "Tailwind",
      // "React",
      // "Redux",
      // "Node js",
      "Gin",
      "Postger sql",
    ],
    about_this_app:
      "A Real Time Full Stack SocalMedia application User Can Upload Post With Image , User Can Make Post CRUD Operations & Reaction Feature , Comment CRUD & Contain User Profile CRUD and Friends Add , Remove RealTime Messanger Can send Image,Link,Text & RealTime With Pusher ",
    design_blog:
      "Basic Desing From Youtube and I Modified That Desing All Code By Me an My Own Desing ",
    dependencies_blog:
      "You can use Yarn or NPM to install the dependencies. In this case I am use Npm",
    package: [
      "@mui/icons-material",
      "@reduxjs/toolkit",
      "formik",
      "javascript-time-ago",
      "jwt-decode",
      "material-ui-popup-state",
      "pusher-js",
      "react-dropzone",
      "react-easy-crop",
      "react-icons",
      "react-redux",
      "react-router-dom",
      "redux-persist",
      "swiper",
      "uuid",
    ],
    conclusion_blog: "",
    youtube_link: "https://youtu.be/cLKsNVSyPMM",
    githhub_link: "",
    frontend_dowload_link:
      "/GrapeMedia/archive/refs/heads/main.zip",
    backend_dowload_link:
      "/SocialMediaBackend-/archive/refs/heads/main.zip",
    project_image: "/wresling.png",
    project_blog:
      "Bihar Wresling State Champion looking to raise funds for better nutrition and living",
    Type: "React",
    indevelopment: false,
  },
  {
    title: "Profiol Server",
    video_key: "",
    say_xi: "",
    say_xi_blog: "",
    technology_feature: ["Golang", "Gin", "Mongo db", "JWT"],
    about_this_app: "",
    design_blog: "",
    dependencies_blog: "",
    package: [],
    conclusion_blog: "",
    youtube_link: "",
    githhub_link: "",
    frontend_dowload_link: "",
    backend_dowload_link: "",
    project_image: "/goserver.png",
    project_blog:
      "My First Go , Gin Server , Basic CRUD Go Server With Mongodb",
    Type: "server",
    indevelopment: true,
  },
  {
    title: "ShoeEcommerce Server",
    video_key: "",
    say_xi: "",
    say_xi_blog: "",
    technology_feature: [
      "Node js",
      "Express js",
      "Postger sql",
      "Firebase",
      "JWT",
      "Stripe",
    ],
    about_this_app: "",
    design_blog: "",
    dependencies_blog: "",
    package: [],
    conclusion_blog: "",
    youtube_link: "",
    githhub_link: "",
    frontend_dowload_link: "",
    backend_dowload_link:
      "/Nike_Backend/archive/refs/heads/main.zip",
    project_image: "/n2.png",
    project_blog:
      " User authentication Google auth , Facebook auth , Nike Auth ..more , Mind leven CRUD functionality",
    Type: "server",
    indevelopment: false,
  },
  {
    title: "FoodOrder & AdminDashboard",
    video_key: "",
    say_xi: "",
    say_xi_blog: "",
    technology_feature: [
      "Node js",
      "Express js",
      "Firebase",
      "Graphql",
      "Stripe",
      "JWT",
    ],

    about_this_app: "",
    design_blog: "",
    dependencies_blog: "",
    package: [],
    conclusion_blog: "",
    youtube_link: "",
    githhub_link: "",
    frontend_dowload_link: "",
    backend_dowload_link:
      "/DeshboardBackend/archive/refs/heads/main.zip",
    project_image: "/n2.png",
    project_blog:
      "User authentication , Graphql  Mind leven CRUD functionality , Google auth , Facebook auth , Fastfood Auth,",
    Type: "server",
    indevelopment: false,
  },
  {
    title: "Deliveroo Server",
    video_key: "",
    say_xi: "",
    say_xi_blog: "",
    technology_feature: [
      "Node js",
      "Express js",
      "Firebase",
      "Postger sql",
      "JWT",
    ],

    about_this_app: "",
    design_blog: "",
    dependencies_blog: "",
    package: [],
    conclusion_blog: "",
    youtube_link: "",
    githhub_link: "",
    frontend_dowload_link: "",
    backend_dowload_link:
      "/Deliveroo/archive/refs/heads/main.zip",
    project_image: "/n2.png",
    project_blog:
      "Node js Express With Postger Sql ,FireBase FireStore  CRUD functionality",
    Type: "server",
    indevelopment: false,
  },
];

export const colour: string[] = [
  "#F59E0B",
  "#65A30D",
  "#EA580C",
  "#C026D3",
  "#14B8A6",
  "#E11D48",
];

/*
  {
    title: "",
    video_key: "",
    say_xi: "",
    say_xi_blog: "",
    technology_feature: [],
    about_this_app: "",
    design_blog: "",
    dependencies_blog: "",
    package: [],
    conclusion_blog: "",
    youtube_link: "",
    githhub_link: "",
    frontend_dowload_link: "",
    backend_dowload_link: "",
    project_image: "",
    project_blog: "",
    Type: "Next js 13",
    indevelopment: false,
  },
*/
